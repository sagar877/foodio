  	...
  	return (
		<div className="bg-lime-600 bg-opacity-5 flex items-center h-16 border-b justify-between shadow-sm px-10">  
			...
			<div className='max-[500px]:hidden flex items-center gap-3'>
				{islog ? <button className='bg-green-700 text-sm px-5 h-8 text-white rounded-xl' onClick={() =>setIslog(false) }>Log In</button>:<button className='bg-red-700 h-8 text-sm px-5 h-8 text-white rounded-xl' onClick={() =>setIslog(true) }>Log Out</button>} 
				<Link to="/cart">
